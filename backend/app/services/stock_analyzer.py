...
def _lookup_name(repo, symbol: str) -> str:
    """从 instruments 缓存取股票中文名(供联网检索提示更精准)。取不到返回空串。"""
    try:
        inst = repo.get_instruments()
        if inst.is_empty() or "name" not in inst.columns or "symbol" not in inst.columns:
            return ""
        hit = inst.filter(pl.col("symbol") == symbol)
        if hit.is_empty():
            return ""
        return str(hit["name"][0] or "")
...
_SYSTEM_PROMPT_TEMPLATE = """你是一位拥有 15 年 A 股一线实战经验的资深交易员兼技术分析师,\
擅长从 K 线、量价、关键价位与基本面交叉验证中把握买卖时机。你的任务是:\
基于提供的个股数据,产出一份**实战、可直接指导交易决策**的综合分析报告。
...
### 1. 🎯 一句话定调(1-2 句)
用一句话概括该股当前的**技术状态与交易属性**(如"高位放量滞涨,需警惕回调"/"底部筹码集中,\
放量突破在即")。结尾用【操作建议:观望 / 轻仓试探 / 逢低吸纳 / 持有 / 减仓 / 规避】给出明确倾向。
...
## 重要免责
报告末尾附一行:"> ⚠️ 本报告由 AI 基于公开行情与财务数据生成,仅供参考,\
不构成任何投资建议。交易有风险,入市需谨慎。"

现在请基于下方数据进行分析。"""


# 消息面第 5 节的两种版本(据联网检索开关二选一填入 __NEWS_SECTION__)。
_NEWS_SECTION_INFER = """**注意:本期无直接新闻数据输入。** 请基于 K 线的**异动信号**进行推断(如:
- 涨停/连板/炸板 → 可能有利好或资金炒作
- 放量暴跌 → 可能有未公开利空
- 突破放量 → 可能有催化剂
明确标注"[推断]",告诉用户这是基于价量的推测,真实消息面数据待接入。\
若无明显异动,直说"近期价量平稳,无明显消息面信号"。"""

_NEWS_SECTION_SEARCH = """**你已具备联网检索能力,请务必联网检索该标的最近 1-2 周\
(以当前日期为基准)的真实信息**,涵盖:
- 公司公告(分红/回购/增减持/业绩预告/重大合同/诉讼等)
- 财经新闻、券商研报观点、行业政策
- 市场舆情/热点讨论
要求:
1. 列出 2-4 条**最有信息量**的条目,每条标注**日期**并**在正文内保留来源链接**(Markdown 链接)。
2. 结合消息面对股价的潜在影响简要点评,与技术面交叉印证。
3. **严禁编造**——检索不到就如实说明"未检索到该标的近期公开消息",不要杜撰新闻或链接。"""


def _system_prompt(live_search: bool) -> str:
    section = _NEWS_SECTION_SEARCH if live_search else _NEWS_SECTION_INFER
    return _SYSTEM_PROMPT_TEMPLATE.replace("__NEWS_SECTION__", section)
...
def _build_user_prompt(
    ...
    if live_search:
        who = f"{name}({symbol})" if name else symbol
        parts.extend([
            "",
            f"请联网检索 {who} 最近 1-2 周的真实新闻/公告/研报/舆情,"
            "用于第 5 节消息面(附日期与来源链接,勿编造)。",
        ])
    return "\n".join(parts)
...
async def analyze_stock_stream(
    ...
    try:
        from app.services.ai_provider import live_search_enabled, stream_ai_text

        # 联网检索开关(全局):决定消息面用真检索还是价量推断
        live_search = live_search_enabled()
        name = _lookup_name(repo, symbol) if live_search else ""

        kline_tail = _clean_rows(df, _KLINE_KEEP_COLS)
        user_prompt = _build_user_prompt(
            kline_tail, fins, levels, close, symbol, focus,
            name=name, live_search=live_search,
        )
        async for delta in stream_ai_text(
            [
                {"role": "system", "content": _system_prompt(live_search)},
